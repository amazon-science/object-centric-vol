...
def do_vid_evaluation_percent(dataset, predictions, ground_truth, output_folder, motion_specific=False, percent = (50,)):
    pred_boxlists = []
    gt_boxlists = []
    ...
    mAP = {motion_index: [] for motion_index in range(len(motion_name))}
    for iou_percent in percent:
        ...
        result_str = ""
        template_str = 'AP{} | motion={:>6s} = {:0.4f}\n'
        for motion_index in range(len(motion_name)):
            result_str += template_str.format(iou_percent, motion_name[motion_index], result[motion_index]["map"])
            mAP[motion_index].append(result[motion_index]["map"])
        print("\n" + result_str)
        if output_folder:
